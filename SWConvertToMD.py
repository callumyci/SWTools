...
def convert_to_markdown(input_file, output_dir):
    ...
    try:
        result = subprocess.run(
            ['pandoc', input_file, '-o', output_file, '--lua-filter=underline-to-italic.lua'],
...
def generate_yaml_metadata(row):
    ...
    metadata_dict = {
        'layout': 'page',  # Default layout for Jekyll
        'title': row['Title of Doc'],  # Document title from spreadsheet
        'permalink': "/output_documents/" + os.path.splitext(row['File Name'])[0].replace(" ", "_") + ".html",  # Permalink (use filename without extension, replacing spaces with underscores)
